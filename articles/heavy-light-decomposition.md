...
* 사실 1: 한 정점에서 자손으로 내려가는 무거운 간선이 두개일 수는 없다. 두 자손의 무게가 각각 절반 이상이라면 모순이 되기 때문이다. (만약 두 자손의 무게가 N으로 같다고 하더라도, 이 경우 부모의 무게는 2N+1이므로 절반이 될 수 없다.)
...
* **Query 연산**: 두 정점을 잇는 경로를 무거운 경로의 조각들로 쪼갠다. 이 조각들은 이렇게 생겼을 것이다: [무거운 경로 번호, 경로 내에서 첫 번째 간선의 번호, 경로 내에서 마지막 간선의 번호]. 조각들을 순회하며 해당 무거운 경로에 대해 만들어 둔 구간 트리를 이용해 해당 구간의 최대치를 찾고, 이 중 최대치를 반환한다.

## HLD 계산하기

HLD는 실제로 어떤 값들을 계산할까? 다양한 방법이 있겠지만, 여기에서 제공하는 예제 구현은 트리가 주어질 때 다음과 같은 값들을 계산한다.
...
임의의 간선을 가리키기 위해 자손의 번호를 사용한다는 데 유의하자. 부모에서 자손으로 가는 간선은 여러 개가 있을 수 있지만, 한 자손에서 부모로 가는 간선의 번호는 유일하기 때문에 자손의 번호를 이용하면 간선과 1:1 관계를 쉽게 유지할 수 있다. 
